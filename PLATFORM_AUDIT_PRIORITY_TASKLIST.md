...
- [~] 131. **Admin:** Add pagination and index awareness for high-volume admin views. *(ACR-0025 and its six reserved decisions were approved by Kevin on 2026-07-13. The implementation, focused/full tests, typecheck, build, route-access gates, generated 70-row index catalog, and read-only local `$indexStats` observation are green. All present in-scope local collections currently report only `_id_`; this is honestly surfaced as missing and no index was created. Trusted narrow/wide browser visual QA remains the sole completion gate because the in-app browser bridge rejected this task as untrusted.)*
