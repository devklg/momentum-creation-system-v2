...
- [~] 124. **Ivory:** Add generated-output audit records with prompt version, input, user, and compliance result. *(Implementation merged in PR #315, but activation/completion remains blocked behind proposed ACR-0024 and safe non-production persistence read-back. Post-merge review also found and prepared corrections for rejected-event taxonomy and template/input identity coupling.)*
...
| 2026-07-13 | P2 generated-output audit governance reconciliation | Codex | Reopened P2-124 after PR #315 merged before independent authority review finished. The implementation is privacy-minimal and automated gates passed, but it adds a persistent audit write and compliance-sensitive fail-closed response boundary without the required High-risk ACR. Post-merge review also found `prompt.output.rejected` was taxonomized as `succeeded` and prompt/input identities were not coupled at runtime; narrow corrections and tests are prepared. | ACR-0024 is Proposed. Kevin approval plus a safe non-production canonical persistence read-back is required before P2-124 can return to complete status. No production mutation or live generated-output test is authorized. |
