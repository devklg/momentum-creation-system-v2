...
- [ ] 135. **Knowledge:** Add stale knowledge correction and supersession workflow. *(ACR-0029 proposed. The recommended bundle extends the ratified Knowledge Evolution runtime into the live `mcs_knowledge_sources`/chunks path with immutable logical-source versions, canonical per-correction Kevin decisions, staged exclusive retrieval cutover, exact cross-store readback, idempotent resume, and rollback. Approval authorizes implementation/read-only verification only; every live correction remains a separate exact-evidence Kevin decision. No automatic staleness judgment, LLM truth decision, or live source mutation is authorized by the proposal.)*
