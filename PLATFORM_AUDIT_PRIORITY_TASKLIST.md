...
- [~] 132. **Context:** Add caching and batching for Context Manager and GraphRAG retrieval canaries. *(Kevin approved ACR-0026 on 2026-07-14. Implementation is in progress for the bounded five-second approved-reference cache, exact-key single-flight coalescing, governed invalidation, 50-id GraphRAG readiness batching, and content-free performance diagnostics. The approved boundary preserves per-request Context Packets, scope isolation, fail-closed readiness, and all activation flags. Focused baseline: 51 tests green.)*
