...
- [~] 131. **Admin:** Add pagination and index awareness for high-volume admin views. *(ACR-0025 and its six reserved decisions were approved by Kevin on 2026-07-13. Implementation is in progress on `codex/p2-131-admin-pagination`: additive keyset pagination, one-release BA compatibility, server-side indexed search/sort, honest `hasMore`, page-first Resource ordering, and verify/report-only index awareness. No index creation or production mutation is authorized.)*
