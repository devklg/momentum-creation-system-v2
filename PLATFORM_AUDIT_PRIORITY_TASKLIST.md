...
- [x] 107. **Follow-up:** Build a unified follow-up queue.
...
| 2026-07-13 | P2 unified follow-up queue | Codex | Closed item 107: added one BA-owned read queue across prospect callbacks, VM/RVM inbound callbacks, prospect reminders (including Event Center-connected reminders), and VM/RVM lead reminders. The queue deduplicates by entity, orders raised hands then overdue/upcoming reminders, links to the owning workspace, and never automates contact. Verification passed: focused domain/component tests, repo typecheck, production build, full server suite (1,997 passed), and full team suite (56 passed). | Automated in-app browser visual inspection was unavailable because the browser connection was not trusted; populated, empty, unavailable, and click-through render states are covered behaviorally. P2-108 is next. |
