...
| 2026-07-13 | P2 Michael language and fallback authority | Codex | Prepared a versioned shared authority for the existing Michael EN/ES controlled response words, supported-language list, safe-path mapping, and current English-only `.team` runtime-card copy. Server fixtures, validation, catalog selection, orchestration metadata, route validation, and the card now consume that source without changing the pre-authored values or enabling generation/persistence. Focused tests, full server suite (2,068 passed / 19 skipped), full team suite (62 passed), repo typecheck, and production build passed. | ACR-0023 is Proposed/Critical and requires Kevin approval before adoption, merge, or checking P2-122. The in-app visual runner was unavailable, so visual QA remains open and is not claimed. No LLM/system prompt, language selector, dormant admin prompt scaffold, Context Manager guidance, compliance rule, provider, persistence, or production communication was activated or changed. |
