...
- [~] 132. **Context:** Add caching and batching for Context Manager and GraphRAG retrieval canaries. *(ACR-0026 proposes the recommended bounded, five-second approved-reference cache, exact-key single-flight coalescing, governed invalidation, 50-id GraphRAG readiness batching, and content-free performance diagnostics. It preserves per-request Context Packets, scope isolation, fail-closed readiness, and all activation flags. Focused baseline: 51 tests green. Awaiting Kevin approval before implementation.)*
