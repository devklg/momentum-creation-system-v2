...
- [~] 126. **PMV:** Build a PMV dashboard without earnings or placement claims. *(Implementation and automated verification are complete on `codex/p2-126-pmv-dashboard`. Independent review corrected terminal-outcome activity inflation, concept grouping, and an unauthorized metric. Visual QA remains blocked because the in-app browser rejected its untrusted automation bridge; no visual pass or screenshot evidence is claimed. See `engineering/audits/p2-126-visual-qa/README.md`.)*
...
- [x] 128. **Admin:** Add admin bottleneck reports for invitations, CRM, training, events, and delivery. *(The existing admin Reports surface composes bounded current-state invitation, CRM integrity, aggregate training, Event Center, and VM/RVM delivery/queue authorities into one failure-isolated aggregate snapshot. Every section states its coverage boundary; the report exposes no person rows or scoring, audits aggregate section status only, and preserves the P2-109 reminder-governance dependency. Automated gates passed, and Kevin approved the contained desktop/mobile component previews on 2026-07-13; the trusted in-app route browser remained unavailable and is not represented as having passed.)*
...
- [x] 131. **Admin:** Add pagination and index awareness for high-volume admin views. *(ACR-0025 and its six reserved decisions were approved by Kevin on 2026-07-13. The implementation, focused/full tests, typecheck, build, route-access gates, generated 70-row index catalog, read-only local `$indexStats` observation, and GitHub CI are green. All present in-scope local collections currently report only `_id_`; this is honestly surfaced as missing and no index was created. The in-app browser bridge rejected trusted narrow/wide visual QA as untrusted; Kevin explicitly accepted and overrode that unavailable gate and authorized PR #332 to merge. The visual inspection is not represented as having passed.)*
- [x] 132. **Context:** Add caching and batching for Context Manager and GraphRAG retrieval canaries. *(Kevin approved ACR-0026 on 2026-07-14. The bounded five-second/128-entry approved-reference cache now provides exact scoped single-flight coalescing, copied results, deterministic LRU, expiry, and governed-write generation invalidation without caching Context Packets or degraded/empty results. GraphRAG readiness batches at most 50 unique ids with stable caller order, set-oriented Mongo/outbox/Neo4j reads, exact collection-grouped Chroma gets, truncation detection, and independent fail-closed outcomes. Content-free counters are exposed on the admin Knowledge surface. Automated verification and GitHub CI are green. The trusted in-app browser bridge was unavailable; Kevin explicitly accepted and overrode that unavailable visual gate and authorized PR #334 to merge. The visual inspection is not represented as having passed.)*
- [x] 133. **Chroma:** Add Chroma re-index tooling and age-out policy. *(Kevin approved ACR-0027 on 2026-07-14. The verified manifest-governed CLI provides bounded `audit`, `reindex`, and `age-out` modes; defaults to dry-run; rebuilds only retrieval-ready approved Knowledge Evolution projections from canonical Mongo through the existing reindex service; and removes only canonically archived or superseded projections. Exact collection allowlists, resumable cursors, complete-scan checks, duplicate/blocked evidence checks, content-free reports, SHA-256 evidence, dedicated decision readback, and post-mutation readback fail closed. The live read-only dry runs observed 60 collections, the same seven unowned exclusions, zero rows for the sampled active collection, and zero mutations. No calendar TTL or live mutation is authorized; any live apply requires a separate Kevin decision bound to the exact mode, collections, and reviewed dry-run SHA-256.)*
- [x] 134. **Knowledge:** Add source conflict detection. *(Kevin approved ACR-0028 on 2026-07-14. The bounded, deterministic, content-free observer and Kevin-only admin integrity warning are implemented and hardened with collection inventory, resource-lineage reconciliation, and malformed/orphaned/ambiguous/incomplete evidence checks. Twenty-five focused assertions, 2,184 passing server tests, 48 passing admin tests, typecheck, build, generated catalogs, route-access gates, GitHub CI, and a live read-only 209-source/209-resource complete scan are green with zero conflicts and zero mutations. The trusted in-app browser rejected the isolated preview as untrusted; visual QA is not represented as passed. Kevin explicitly accepted the automated UI evidence as the substitute for that unavailable gate and authorized PR #339 to merge. No source mutation, semantic LLM judgment, correction, lifecycle change, or supersession is authorized.)*
- [ ] 135. **Knowledge:** Add stale knowledge correction and supersession workflow. *(ACR-0029 proposed. The recommended bundle extends the ratified Knowledge Evolution runtime into the live `mcs_knowledge_sources`/chunks path with immutable logical-source versions, canonical per-correction Kevin decisions, a staged safe-gap/exclusive retrieval cutover, exact cross-store readback, idempotent resume, additive `superseded_authority` history, and rollback through a new monotonic version rather than historical-row reactivation. Approval authorizes implementation/read-only verification only; every live correction remains a separate exact-evidence Kevin decision and any live index apply remains separately gated. No automatic staleness judgment, LLM truth decision, or live source mutation is authorized by the proposal.)*
