...
**Learning notes and anchors (ACR-0012, ratified 2026-07-11):** durable agent lessons live in `universal_gateway.claude_learning_notes` on the **memory stack** (gateway connectors `mongodb`/`chromadb`/`neo4j`) — never the app-stack connectors (`mongodb2`/`chromadb2`/`neo4j2`), which host a same-named `momentum` database and accept wrong-stack writes silently. Write through `writeAgentNote()` / `writeAnchor()` in [server/src/lib/agentMemory.ts](server/src/lib/agentMemory.ts) (canonical schema, four lowercase severities, Chroma delete-then-add, read-back on all three legs; anchors are minted only when Kevin names them and must retrieval-test). Spec: [docs/governance/ACR-0012-agent-memory-schema.md](docs/governance/ACR-0012-agent-memory-schema.md). Regenerate the context index with `pnpm memory:index` (read-only).
