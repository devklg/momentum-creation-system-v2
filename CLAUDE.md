...
**Learning notes and anchors (ACR-0012, ratified 2026-07-11):** durable agent lessons live in `universal_gateway.claude_learning_notes` on the **memory stack** (gateway connectors `mongodb`/`chromadb`/`neo4j`) — never the app-stack connectors (`mongodb2`/`chromadb2`/`neo4j2`), which host a same-named `momentum` database and accept wrong-stack writes silently. Write through `writeAgentNote()` / `writeAnchor()` in [server/src/lib/agentMemory.ts](server/src/lib/agentMemory.ts) (canonical schema, four lowercase severities, Chroma delete-then-add, read-back on all three legs; anchors are minted only when Kevin names them and must retrieval-test). Spec: [docs/governance/ACR-0012-agent-memory-schema.md](docs/governance/ACR-0012-agent-memory-schema.md). Regenerate the library of context with `pnpm memory:index` (read-only; emits `docs/memory-index.html` + `docs/memory-drift-report.md` across ALL memory stores).

**Context retrieval and the guard (ACR-0013/ACR-0014, ratified 2026-07-11):** before proposing new work on any topic, run the guard — `pnpm memory:guard "<topic>"` (`checkExisting()` in [server/src/lib/contextGuard.ts](server/src/lib/contextGuard.ts)) — which searches every store in ACR-0013 §3 and returns hits with provenance (store, record, date, who stated it: Kevin or agent). Retrieval follows the ladder in [docs/governance/ACR-0013-context-retrieval-standard.md](docs/governance/ACR-0013-context-retrieval-standard.md): exact `call_phrase`/alias invocation first, then compiled packet (`pnpm memory:packet "<phrase>"`), then union semantic fallback across all stores — never one collection. Absence discipline: "I don't have that" is sayable only after all stores were searched and reachable. **Only Kevin mints handles** — `writeHandle()` in [server/src/lib/memoryContextIndex.ts](server/src/lib/memoryContextIndex.ts) enforces `named_by` and retrieval-tests every phrase; a handle that does not retrieve fails the build (`pnpm memory:verify`, plus the vitest retrieval regression). The context agent (guard → parse → propose → confirm → close) is [server/src/lib/contextAgent.ts](server/src/lib/contextAgent.ts) per [docs/governance/ACR-0014-context-agent.md](docs/governance/ACR-0014-context-agent.md) — it proposes to `mcs_learning_candidates`; Kevin confirms, weights, and names.
