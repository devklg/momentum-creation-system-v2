...
| `ba_commitments` | `commitmentId` | `commitmentId`·`baId`·`threeBaId`·`email`·`version`·`acceptedAt` | → member | One-click commitment acceptance (audit-grade). |
| `ba_questionnaires` | `questionnaireId` | `questionnaireId`·`baId`·`threeBaId`·`version`·`submittedAt` + 21 enum answers | → member | Self-serve interview (employmentStatus, weeklyHours, coachability, nwmExperience, …). |
