...
- **Automatic lifecycle delivery — ACR-0017.** Project hooks in `.codex/hooks.json` and `.claude/settings.json` inject `knowledge/CONTINUATION_CONTEXT.md` plus unread Intervector context at `SessionStart`, run the guard automatically on the first `UserPromptSubmit`, and seed `SubagentStart`. If a hook reports unavailable, that is not verified absence: run `pnpm memory:guard "<topic>"` manually before substantive work. Codex requires one-time trust of the project hook definition through `/hooks` after it changes.
