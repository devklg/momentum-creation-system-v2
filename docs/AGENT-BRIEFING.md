...
- **Automatic lifecycle delivery — ACR-0017.** Project hooks in `.codex/hooks.json` and `.claude/settings.json` validate the continuation's declared `origin/main` base and bounded tracked-revision freshness, inject a verified-current `knowledge/CONTINUATION_CONTEXT.md` plus unread Intervector context at `SessionStart`, run the guard automatically on the first substantive `UserPromptSubmit`, and seed `SubagentStart`. Stale or unversioned task pointers are withheld with an explicit warning. If a hook reports unavailable, that is not verified absence: run `pnpm memory:guard "<topic>"` manually before substantive work. Codex requires one-time trust of the project hook definition through `/hooks` after it changes.
