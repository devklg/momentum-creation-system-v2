...
- 10DLC brand submitted: id `4b20019f-2a24-78ef-cabd-d44042e2e90b`, status UNVERIFIED — **UNBLOCKED 2026-07-08: corrected EIN in hand.** The fix arrived as a NEW EIN for a renamed entity — CP575A dated 2026-07-07, legal name exactly `TEAM MAGNIFICENT MARKETING` (the 2026-07-03 EIN under the typo name `TEAM MARGNIFICENT MARKETING GROUP` is superseded for TCR purposes; close that unused IRS account — confirm with tax pro — so its 1120 obligation doesn't dangle). EIN values are NOT in this file per §4 — canonical copies are the two CP575A PDFs in Kevin's records. NEXT: update/resubmit brand with new EIN + exact name → campaign (Low Volume Mixed; description = account notifications + personally-requested links, opt-in, STOP honored) → assign to MCS-VM-App → re-run SMS smoke.
