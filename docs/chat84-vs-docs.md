...
14. **One server, three clients, three databases, one external tooling.** Admin Dashboard (Kevin only), `.team` client (BA-facing), `.com` client (prospect-facing). All three call one Node + Express + TypeScript server. Every write flows through the external MCP tooling at `localhost:2525` to MongoDB (system of record), Neo4j (operational genealogy graph), and ChromaDB (semantic search).
