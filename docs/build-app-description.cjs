...
const children = [
  ...
  h2('5.2  Steve and Michael \u2014 discovery plus daily success coaching'),
  p('Steve conducts Discovery and creates a non-scored Success Profile. Michael is an outbound voice agent (Telnyx-based). When a new BA arrives, Michael calls them as the Training Agent and Daily Success Coach to capture:'),
  ...
  p('The transcript and training/daily-success context are captured and fed back to the BA\u2019s own record and to the sponsor BA (their upline). Steve and Michael do not score, rank, classify, or predict the BA. Michael is BA-facing only \u2014 never prospect-facing, never anywhere on .com.'),
