...
adminKnowledgeRoutes.post('/sources', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    return res.status(201).json({
      ...
      taxonomy: (result.source as { taxonomy?: unknown }).taxonomy,
      categoryTags: (result.source as { categoryTags?: string[] }).categoryTags ?? [],
      productTags: (result.source as { productTags?: string[] }).productTags ?? [],
      canonicalTopicTags: (result.source as { canonicalTopicTags?: string[] }).canonicalTopicTags ?? [],
