 ...
 * gateway calls MADE BY the live-ops domain functions — not the global
 * gateway-call population. This is the "honest-partial" pattern from
...
/** Record a single gateway round-trip latency in milliseconds. */
...
export async function instrumentedGatewayCall<T = unknown>(
  ...
  try {
    return await gatewayCall<T>(tool, action, params);
  } finally {
