...
describe('S2.12 Michael response contract static governance boundary', () => {
  it('keeps Michael response contract imports away from stores, GraphRAG, adapters, Gateway fallback, persistence, and retrieval helpers', () => {
    const forbiddenImports =
      /\bfrom\s+['"][^'"]*(?:^|\/|\\|@)(?:mongoose|mongodb|neo4j-driver|chromadb|openai|anthropic|@anthropic-ai)(?:$|\/|\\|['"])|\bfrom\s+['"][^'"]*(?:graph-?rag|\/services\/gateway|\/services\/persistence|\/persistence\/|gatewayFallback|gateway-fallback|rawRetrieval|retrievalHelper|directRetrieval|retrieval|\/services\/anthropic|\/services\/openai)[^'"]*['"]/i;
  ...
  it('preserves Gateway fallback client outside Michael response contract source', () => {
