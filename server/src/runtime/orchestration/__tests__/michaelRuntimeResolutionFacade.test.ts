...
/**
 * Build the MichaelRuntimeAdapterContractInput exactly like the S2.15 adapter
 * contract test's `runContract` builder, then resolve it through the facade's
 * adapter-input entry point. Returned-only; mutates nothing it does not own.
 */
...
async function resolve(
  overrides: ResolveOverrides = {},
): Promise<MichaelRuntimeResolutionResult> {
  return resolveMichaelRuntimeTurnResponseFromAdapterInput(await buildAdapterInput(overrides));
}
...
// Copied verbatim from michaelRuntimeAdapterContract.test.ts — flips the packet
// assembler away from the Context Manager so derivation derives a safe-close.
...
describe('S2.20 Michael runtime resolution facade — adapter-input', () => {
  ...
  it('12. resolves ES turns to ES catalog entries', async () => {
    const cases: ResolveOverrides[] = [
  ...
  it('19. does not mutate the runtime turn it resolves', async () => {
    const input = await buildAdapterInput({ intent: 'clear_training_support' });
    const before = JSON.stringify(input.runtimeTurn);

    resolveMichaelRuntimeTurnResponseFromAdapterInput(input);

    expect(JSON.stringify(input.runtimeTurn)).toBe(before);
  });
