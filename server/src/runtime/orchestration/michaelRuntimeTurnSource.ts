 ...
 *                     the sanctioned CONTEXT-LAYER factory. Orchestration NEVER
 *                     assembles a packet; the Context Manager remains the only
 *                     assembler;
 *  - accepted / fail-closed — a complete Context Manager packet may proceed to
 *                     the controlled Michael catalog; a degraded packet resolves
 *                     to the pre-authored `safe_fallback` response. On any
 *                     assembly/coordination failure it returns a deterministic
 *                     typed-issues result — it never throws, never persists,
 *                     never emits a partial unsafe turn;
...
/**
 * Build a fail-closed Michael runtime turn for the authenticated BA.
 *
 ...
 * turn coordinator. Live approved-knowledge retrieval, when enabled, stays in
 * the context layer behind that port.
 */
export async function createMichaelRuntimeTurnForAuthenticatedBa(
  ...
  // Production Context Manager port — built by the sanctioned context-layer
  // factory. The orchestration layer does not assemble packets and does not
  // import store/retrieval clients; it only injects the returned port.
  ...
  // Fail closed: only complete/proceed and degraded packets may reach the
  // controlled Michael response catalog. Rejections and failed/blocked packets
  // return typed issues and NO turn rather than a partial unsafe turn.
  if (result.decision !== 'proceed' && result.decision !== 'degraded') {
    return {
      ok: false,
      issues: [
        {
          ...
          message:
            `Expected an accepted or degraded runtime turn but the coordinator ` +
  ...
  const scenario = result.decision === 'proceed' ? 'accepted_complete' : 'accepted_degraded';
  const metadata: RuntimeTurnFixtureScenarioMetadata = {
    scenario,
    description:
      result.decision === 'proceed'
        ? 'Server-owned, session-scoped Michael runtime turn (complete, controlled catalog).'
