...
const INDEX_PLAN = [
  ...
  idx('tmag_prospect_htank_placements', 'prospect_flushed_placedAt', { prospectId: 1, flushedAt: 1, placedAt: -1 }, {}, 'One live placement lookup while preserving historical flushed attempts.'),
  idx('tmag_prospect_htank_placements', 'unique_placementId', { placementId: 1 }, { unique: true, partialFilterExpression: { placementId: { $type: 'string' } } }, 'Konga placement identity; legacy rows remain valid.'),
  idx('tmag_prospect_htank_placements', 'unique_placementAttemptId', { placementAttemptId: 1 }, { unique: true, partialFilterExpression: { placementAttemptId: { $type: 'string' } } }, 'One placement per immutable invitation attempt; legacy rows remain valid.'),
  ...
  idx('tmag_konga_page_visits', 'token_observedAt', { tokenHash: 1, observedAt: -1 }, {}, 'Token-bound visit delta and idempotent reconnect lookup.'),
  idx('tmag_konga_replay_pointers', 'active_availableAt', { publicationStatus: 1, availableAt: -1 }, {}, 'Current authorized replay pointer.'),
  idx('tmag_konga_replay_completions', 'token_completedAt', { tokenHash: 1, completedAt: -1 }, {}, 'Replay completion and callback unlock readback.'),
  idx('tmag_konga_enrollment_attestations', 'prospect_enrollee_status', { prospectId: 1, enrolleeTmagId: 1, status: 1 }, { unique: true }, 'Idempotent human enrollment attestation.'),
  idx('tmag_konga_enrollment_attestations', 'sponsor_joinedAt', { sponsorTmagId: 1, joinedAt: 1 }, {}, 'Report-only mission funnel source.'),
  ...
  idx('tmag_event_attendance', 'event_recordedAt', { eventId: 1, recordedAt: -1 }, {}, 'Event attendance aggregation by event.'),
  idx('tmag_event_attendance', 'reservation_recordedAt', { reservationId: 1, recordedAt: -1 }, {}, 'Latest explicit attendance state per reservation.'),
  ...
  idx('mcs_knowledge_corrections', 'unique_knowledge_correction_idempotency', { idempotencyKey: 1 }, { unique: true }, 'Bind one correction workflow to one immutable apply key.', 'required_not_verified'),
  idx('mcs_knowledge_corrections', 'knowledge_correction_state_updated', { state: 1, updatedAt: 1 }, {}, 'Bound retry and operations status scans.', 'required_not_verified'),
  idx('mcs_knowledge_sources', 'knowledge_source_version_list', { status: 1, createdAt: -1, _id: -1 }, {}, 'Kevin-only keyset source-version list.', 'required_not_verified'),
  idx('mcs_knowledge_chunks', 'knowledge_chunk_source_version_lifecycle', { sourceId: 1, sourceVersion: 1, status: 1 }, {}, 'Exclusive old/new chunk lifecycle readback.', 'required_not_verified'),
...
const HIGH_VOLUME_COLLECTIONS = new Set([
  ...
  'tmag_konga_page_visits',
  'tmag_konga_replay_pointers',
  'tmag_konga_replay_completions',
  'tmag_konga_enrollment_attestations',
  'tmag_prospect_htank_accounts',
  'tmag_prospect_magic_links',
  'tmag_prospect_callback_requests',
  'tmag_prospect_webinar_reservations',
  'tmag_event_attendance',
