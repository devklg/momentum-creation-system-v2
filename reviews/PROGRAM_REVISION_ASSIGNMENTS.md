...
| ~~R-3~~ | — | `MOMENTUM_CONSTITUTION.md` | ~~Michael note not verified~~ **RESOLVED on full read** | Note is present, binding, in **Article VIII.3** (C-4) | No action | — | — | done |
...
| **R-8** | Domain / QA Agency | Domain specs + `compliance.ts`/`rules.ts` | Code/spec compliance with constitutional boundaries unverified (Conflict Report boundary table) | Boundaries are *defined* (Const. Art. IV/VII) but it is unverified that code fails closed and domain specs match — e.g. five `.com` prohibitions, monotonic Holding Tank position, sponsor-immutability route rejection | Read the ~16 domain specs + enforcement code; verify each boundary is enforced; file defects as ACRs | Medium | L | R-2 |
