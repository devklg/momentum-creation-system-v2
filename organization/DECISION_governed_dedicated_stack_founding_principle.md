...
**One prefix, one root, one word — always applied (Kevin, 2026-07-01).** A concept's name is `mcs`/`Mcs` prefix + one root word + a clear role suffix, and the prefix is applied to **every** name of that concept — never present on some and absent on others. So the "outcome" concept is `mcs_outcomes` (collection) ⇄ `McsOutcome`, `McsOutcomeKind`, `McsOutcomeRecord`, `McsOutcomeInput` (types) — same root, prefix always on. Forbidden: un-prefixed siblings (`AppendOutcomeInput` next to `McsOutcomeRecord`) or a shifting root (`McsCandidateReview` vs `McsLearningCandidateRecord`). Two transforms are deterministic and allowed: **collection plural / type singular** (a collection holds many; a type describes one) and **casing per layer** (below). Everything else is one word.
