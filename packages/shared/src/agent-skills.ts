...
export const MCS_AGENT_SKILL_REGISTRY: readonly McsAgentSkillDescriptor[] = [
  ...
  skill({ skillKey: 'ivory.draft_personal_invitation', ownerAgentKey: 'ivory', version: '1.0.0', status: 'implemented',
    purpose: 'Draft one editable invitation for a person the BA has already identified.', requiredInputs: ['authenticated_ba', 'ba_identified_person', 'ba_authored_relationship_context'],
    allowedOutputs: ['editable_invitation_draft'], forbiddenOutputs: ['automatic_send', 'prospect_selection', 'qualification', 'income_claim', 'medical_claim', 'placement_promise'],
    behaviorSource: 'server/src/domain/ivory.ts#draftIvoryInvitation', templateIds: ['ivory_personal_invitation'],
    testIds: ['ivoryGeneratedCopyCompliance.test.ts', 'ivoryScriptMakerPromptRegistry.test.ts'], handoffTarget: 'brand_ambassador_then_invitation_spine', eventsEmitted: ['ivory.invitation.drafted'],
    degradation: { mode: 'deterministic_fallback', behavior: 'Return a neutral editable invitation without echoing private relationship notes.' } }),
...
export const MCS_AGENT_TEMPLATE_REGISTRY: readonly McsAgentTemplateDescriptor[] = [
  ...
  template({ templateId: 'ivory_wdyk_coach', version: '1.0.0', ownerAgentKey: 'ivory', skillKey: 'ivory.manage_personal_prospect_roster', status: 'active', owner: 'Team Magnificent Leadership', behaviorSource: 'server/src/domain/ivory.ts#buildCoachSystem+buildCoachUserTurn', allowedInputs: ['ba_angle', 'product_name', 'roster_size', 'ba_ask', 'admin_master_content'], allowedOutputs: ['memory_prompts', 'coaching'], forbiddenOutputs: ['scraped_contact', 'prospect_selection', 'qualification', 'automatic_send', 'income_claim', 'medical_claim', 'placement_promise'], degradation: { mode: 'deterministic_fallback', fallbackSource: 'neutralCoach' }, testIds: ['ivoryGeneratedCopyCompliance.test.ts', 'ivoryScriptMakerPromptRegistry.test.ts'], approval: { state: 'approved', authority: 'Kevin via merged PR #206', activatedAt: '2026-05-31' } }),
  template({ templateId: 'ivory_personal_invitation', version: '1.0.0', ownerAgentKey: 'ivory', skillKey: 'ivory.draft_personal_invitation', status: 'active', owner: 'Team Magnificent Leadership', behaviorSource: 'server/src/domain/ivory.ts#INVITATION_DRAFT_SYSTEM+buildInvitationDraftUserTurn', allowedInputs: ['ba_identified_person', 'prospect_first_name', 'relationship_categories', 'private_relationship_context', 'product_context'], allowedOutputs: ['editable_invitation_draft'], forbiddenOutputs: ['automatic_send', 'prospect_selection', 'qualification', 'income_claim', 'medical_claim', 'placement_promise'], degradation: { mode: 'deterministic_fallback', fallbackSource: 'neutralInvitationDraft' }, testIds: ['ivoryGeneratedCopyCompliance.test.ts', 'ivoryScriptMakerPromptRegistry.test.ts'], approval: { state: 'approved', authority: 'Kevin via merged PR #312; current prompt authored in 1cf31f02', activatedAt: '2026-07-03' } }),
  template({ templateId: 'ivory_momentum_followup', version: '1.0.0', ownerAgentKey: 'ivory', skillKey: 'ivory.suggest_follow_up', status: 'active', owner: 'Team Magnificent Leadership', behaviorSource: 'server/src/domain/ivory-momentum.ts#SUGGEST_SYSTEM_PREFIX+buildSuggestUserTurn', allowedInputs: ['owned_prospect', 'pmv_lifecycle', 'last_signal', 'preferred_angle', 'relationship_categories', 'private_relationship_context', 'ba_ask'], allowedOutputs: ['editable_followup_suggestion'], forbiddenOutputs: ['automatic_send', 'prospect_selection', 'qualification', 'pressure', 'income_claim', 'medical_claim', 'placement_promise'], degradation: { mode: 'deterministic_fallback', fallbackSource: 'neutralSuggestion' }, testIds: ['ivoryMomentumGeneratedCopyCompliance.test.ts', 'ivoryScriptMakerPromptRegistry.test.ts'], approval: { state: 'approved', authority: 'Kevin via merged PR #206', activatedAt: '2026-06-23' } }),
  template({ templateId: 'scriptmaker_product_invitation', version: '1.0.0', ownerAgentKey: 'scriptmaker', skillKey: 'scriptmaker.draft_product_invitation', status: 'active', owner: 'Team Magnificent Leadership', behaviorSource: 'server/src/domain/scriptmaker.ts#SYSTEM_PREFIX+resolveSeed+buildUserTurn', allowedInputs: ['script_kind', 'product_name', 'video_title', 'prospect_first_name', 'prospect_context', 'event_day', 'event_time', 'admin_master_content_seed'], allowedOutputs: ['editable_invitation_draft'], forbiddenOutputs: ['automatic_send', 'prospect_selection', 'qualification', 'income_claim', 'medical_claim', 'placement_promise'], degradation: { mode: 'deterministic_fallback', fallbackSource: 'resolved compliant master-content seed or neutralFallback' }, testIds: ['scriptmakerGeneratedCopyCompliance.test.ts', 'ivoryScriptMakerPromptRegistry.test.ts'], approval: { state: 'approved', authority: 'Kevin via merged PR #206', activatedAt: '2026-05-22' } }),
