...
```mermaid
flowchart TD
  Kevin[Kevin L. Gardner — Constitutional Authority] --> Const[THE MOMENTUM CONSTITUTION - living]
  Found[FOUNDATION.md — Founding Charter - historical] -.descends into.-> Const
  Const --> Gov[MOMENTUM_GOVERNANCE.md]
  Const --> Dec[MOMENTUM_DECISION_FRAMEWORK.md]
  Const --> ACR[MOMENTUM_ACR_SYSTEM.md]
  Const --> Pillars[Governance Pillars: SCHEMA / MULTI_DB / AGENT_ARCHITECTURE / PROMPT]
  Const --> Op[Operational Currency: decision ledger to handoffs]
  Gov --> Agents[Agents — advisory only]
  Dec --> Agents
  ACR --> Agents
  Pillars --> Agents
  Agents -. warn, never override .-> Kevin
```
...
stateDiagram-v2
  ...
  Proposed --> Audited
  Audited --> Inventoried
  Inventoried --> Reconciled
  Reconciled --> GapAnalyzed
  GapAnalyzed --> Drafted
  Drafted --> CrossReferenced
  CrossReferenced --> AwaitingRatification
  AwaitingRatification --> Ratified: Kevin approves
  AwaitingRatification --> Revised: Kevin requests changes
  Revised --> Reconciled
  Ratified --> [*]
```
